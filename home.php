 ...
 <div id="home" class="tab-pane fade in active">
  ...
  <label class="control-label col-sm-1" for="status">Status</label>
   ...
   <script>
     ...
     <select id="slt2" class="form-control">
      <option>Select...</option>
      <option value="#tab_4">All</option>
      <option value="#tab_5">Finished</option>
      <option value="#tab_6">Not Finish</option>
      <option value="#tab_7">Cancel</option>
     </select>
     <div class="88" id="tab_4">
     <div class="row">
       <div class="col-xs-8">
         <table class="table" style=" margin-top: 15px;">
           <thead>
  ...
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $UID = $_SESSION["UID"];
  $stmt = $conn->prepare("select OID from orderlist where UID =:UID");
  $stmt->execute(array('UID' => $UID));
  $pid = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'OID');
  $stmt = $conn->prepare("select Status from orderlist where UID =:UID");
  $stmt->execute(array('UID' => $UID));
  $result0 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'Status');
  $stmt = $conn->prepare("select Start from orderlist where UID =:UID");
  $stmt->execute(array('UID' => $UID));
  $result1 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'Start');
  $stmt = $conn->prepare("select End from orderlist where UID =:UID");
  $stmt->execute(array('UID' => $UID));
  $result2 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'End');
  $stmt = $conn->prepare("select Amount from orderlist where UID =:UID");
  $stmt->execute(array('UID' => $UID));
  $result3 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'Amount');

  $stmt = $conn->prepare("select SID from orderlist where UID =:UID");
  $stmt->execute(array('UID' => $UID));
  $sid = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'SID');
  $conn = mysqli_connect($dbservername, $dbusername, $dbpassword, $dbname);
  $sql = "select SID from orderlist where UID = $UID";
  $result = mysqli_query($conn, $sql);
  $shopname = array();
  if($result){
    for($i = 0; $i < mysqli_num_rows($result); $i++){
      $SID = mysqli_fetch_row($result)[0];
      $sql = "select Name from Store where SID = $SID";
      $data = mysqli_query($conn, $sql);
      $row = mysqli_fetch_row($data);
      if($data){
      $sname = $row[0];
  ...
  for($i = 0; $i < count($result0); $i++){
      $show = $show . '<tr>' .
                          ...
                          '<td>' . $shopname[$i] . '</td>' .
                          '<td>' . $result3[$i] . '</td>' .
                          '<td><button type="button" class="btn btn-info" data-toggle="modal" data-target="#test123">order details</button></td>';
      if($result0[$i] == "Not Finished"){
        $show = $show . '<td><button type="submit" id="' . $sid[$i] . '"class="btn btn-danger">Delete</button></td>'.
                        '</tr>' ;
      }
      else{
        $show = $show . '</tr>' ;
  ...
  for($i = 0; $i < count($result0); $i++){
    if($result0[$i] == "Not Finished"){
      $show = $show . '<tr>' .
                          ...
                          '<td>' . $result3[$i] . '</td>' .
                          '<td><button type="button" class="btn btn-info" data-toggle="modal" data-target="#test123">order details</button></td>' .
                          '<td><button type="submit" id="' . $sid[$i] . '"class="btn btn-danger">Delete</button></td>'.
                          '</tr>' ;
    }
  }
  echo $show;
...
?>
 ...
 <div class="row">
  ...
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $UID = $_SESSION["UID"];
  $stmt = $conn->prepare("select RID from record where UID1 =:UID");
  $stmt->execute(array('UID' => $UID));
  $pid = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'RID');
  $stmt = $conn->prepare("select Action from record where UID1 =:UID");
  $stmt->execute(array('UID' => $UID));
  $result0 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'Action');
  $stmt = $conn->prepare("select Time from record where UID1 =:UID");
  $stmt->execute(array('UID' => $UID));
  $result1 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'Time');
  $stmt = $conn->prepare("select Amount from record where UID1 =:UID");
  $stmt->execute(array('UID' => $UID));
  $result3 = get_attr_arr($stmt->fetchAll(PDO::FETCH_CLASS), 'Amount');
  $show = "";
  for($i = 0; $i < count($result0); $i++){
